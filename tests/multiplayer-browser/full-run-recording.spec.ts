...
async function readWorld11SmallInputTrace(): Promise<readonly RecordedInputRun[]> {
  const serialized = await readFile(
    resolve(process.cwd(), "tests/multiplayer-browser/world11-small-input-trace.json"),
    "utf8",
  );
  return JSON.parse(serialized) as readonly RecordedInputRun[];
}

async function replayRecordedWorld11Input(player: RecordedPlayer): Promise<void> {
  await releaseRunningKeys(player);
  await player.page.getByLabel("Authoritative multiplayer game view").focus();
  let held = { left: false, right: false, jump: false, down: false, run: false };
...
async function runAndJumpToExit(players: readonly RecordedPlayer[]): Promise<void> {
  const replayLeader = players[0];
  if (replayLeader === undefined) throw new Error("Recording replay leader is missing.");
